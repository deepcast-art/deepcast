...
**FINAL DESIGN SPEC (2026-07-16, second revision — supersedes the earlier amendment block below where they conflict):**
- **Three pages, one job each; the network idea at three intensities**: lineage thread on the landing letter (whisper) → platform-concept line in the watch page's share panel (the ask) → full graph on the dashboard (the payoff). No graph-reveal beat on the claim path — claim routes DIRECTLY to watch (the earlier reveal beat is retired from the codebase).
- **Page 1 — Landing (/{slug})**: full-bleed film still behind the letter (`films.poster_url`, nullable; defaults to the film's Mux poster frame; NULL-safe to the dark background). Order: Dear X / sharer line (first-name) / lineage thread / film title / transmission hook / inline email field + Accept CTA (no click-to-reveal) / "This invitation admits one person, once." NOT here: concept line, ordinal, conditions line, graph.
- **Ordinal freeze**: `invites.claim_ordinal` (nullable int) stamped once at claim with the invitee's position, never recomputed. Pre-claim ordinals may be computed live; the dashboard shows the frozen value.
- **Page 2 — Watch**: title + conditions line threshold, player, and a permanently-docked share panel (collapsed; expandable any time; pause nudges it; credits-end auto-opens; never overlays the video). Expanded order: the platform-concept line (its primary home) → "You have N tickets for this film. Each admits one person, once." → first-name field → generate → link + copy + ready-to-send line → "See where your ticket went →" (dashboard). Zero tickets: "You've given all your tickets for this film." Persistent quiet dashboard link on the page.
- **Ticket economy (dual backing, one economy)**: tickets spend at LINK GENERATION, no refunds. Account holders: existing `users.invite_allocation` machinery unchanged, surfaced in "tickets" language. Accountless claimants: `invites.tickets_remaining` (nullable int), initialized to **5** at claim (mirrors the uniform new-viewer grant in every account-creation code path), checked + atomically decremented (CAS) by the accountless create-link path — closing the no-quota gap.
- **Page 3 — Dashboard**: the EXISTING Dashboard.jsx adapted, not a new page. Accepts claimed-invite identity alongside auth sessions (claim response stashed via safeStorage; slug as fallback). Accountless mode hides name-edit, sign-out, and About; quota UI reads `tickets_remaining`; "tickets" language viewer-facing; adds the frozen-ordinal line, optional quiet concept line, and amber highlighting of the viewer's own path INCLUDING branches they created.
- **Revisit rule**: a claimant re-opening their own claimed link (recognized by stash) routes to their watch/dashboard state; anyone else gets the dead-link page. No stash (new browser) → dead-link is the accepted MVP fallback — a known limitation, no recovery built.

**Earlier amendment block (2026-07-16, first revision — the graph-reveal-on-claim beat described here is now retired; kept for history):**
