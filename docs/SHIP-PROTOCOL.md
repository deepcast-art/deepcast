...
- **CI:** `https://github.com/deepcast-art/deepcast/actions/workflows/ci.yml` — read it in the founder's Chrome (the anonymous GitHub API refuses the cloud fetch); the run page's Annotations list names the failing spec and line. Documented flake specs (webkit/firefox resume, webkit auth-link) → rerun the job; anything else red is a Claude Code prompt. `main` is deliberately NOT branch-protected (see first tasks, item 2) — CI is a signal the verifier reads, and a red `main` is always the next task.
...
1. ~~Add `.github/workflows/ci.yml`~~ — **DONE 3 September (`5dc8860`, hardened in `3d6cbea` and `e6342df`; run #3 `33711774502` green on all four jobs).** Original brief: on every push and pull request — `npm ci`, `npm run lint`, `npm run test:unit`, `npm run build`, then Playwright e2e on chromium, webkit, and firefox (install browsers in CI with `npx playwright install --with-deps`). Cache node_modules. Keep the run under ~10 minutes; if the e2e suite needs the real database, gate it on a repository secret and document that in the workflow.
2. ~~Turn on branch protection for `main`~~ — **DECIDED AGAINST (verifier, 3 September, after the first CI run):** GitHub's required-status-check protection blocks every push to `main`, including Claude Code's Tier 1 pushes, which would defeat the autonomy the protocol exists for. CI is therefore the independent SIGNAL, not a lock: the verifier reads the run before ever saying "merge" on a Tier 2 PR, and **a red CI run on `main` after a Tier 1 push is automatically Claude Code's next task** — fix or `git revert`, never ignore. Vercel's build gate (which runs the unit tests) still physically prevents a broken build from deploying. Revisit only if a second human ever pushes to this repo.
3. **First-run findings (CI #1, 5dc8860) — BOTH FIXED (`3d6cbea`, `e6342df`; root causes in CLAUDE.md's Testing section):** lint/unit/build green; chromium 76 passed; firefox 76 passed; webkit 1 flaky (`creator-watch-page.spec.js` baseline captured before the mocked payload rendered — passed on retry; the test must wait for the data) and 1 failed (`claim-landing.spec.js` full arc: the CI runner's WebKit blocks Mux's `inferred.litix.io` analytics beacon with an "access control" console error, and the spec's `jsErrors` assertion treats it as an app error — route/abort that host in the test harness or filter that known third-party message; it is the runner's network, not the product). Both are e2e-only, Tier 1.
4. ~~Reconcile CLAUDE.md and `docs/PROJECT-BRIEF.md` to this protocol at the next end-of-day.~~ — **DONE 3 September, end of day** (both files record the protocol, CI runs #1–#3 and their findings, the harness, the ghost removal as executed, the Pro-backup correction, and the unprotected-`main` decision).
