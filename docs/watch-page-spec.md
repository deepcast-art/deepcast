...
**The player is `display: block` (builder fix, 15 September 2026, branch `player-stalls`).** `<mux-player>` is a custom element whose default layout is `inline-block` on the baseline, so inside the `bg-black` wrapper the element sat on a text baseline and the wrapper was ~9.5px taller than the player (measured on production at 1440: wrapper 744×428, player 744×418.5) — that strip was the wrapper's black painting under the film. The player's className carries `block`; `aspect-video` is untouched (the asset is 1920×1080, the ratio was never the problem). `e2e/player-frame.spec.js` measures the wrapper's height against the player's (±0.5px) at 1440 and 390 on all three engines.

**The player preloads and carries an hls.js buffer config (builder fix for the first-minute freezes, 15 September 2026, branch `player-stalls`; measured with `scripts/measure-playback.mjs`).** Symptom: on desktop Chrome playback froze for ~2 s at varying points, mostly in the first minute; phones fine. Measured mechanism (class A — buffer/ABR, not render: zero long tasks, zero dropped frames in every run): under the stock `preload="metadata"` mux caps the pre-play buffer at ONE segment (`maxBufferLength = maxBufferSize = 1` until the first play), so play began on 4.96 s of film and the first ~15 s ran on 3–8 s of buffer; at ~5 s in, ABR stepped straight from the 720p start level to the 1080p @ 7.0 Mbps rendition (the up-switch needs an estimate of only bitrate ÷ 0.7 ≈ 10 Mbps) — the thinnest cushion at the moment of the biggest bitrate. A Retina desktop (DPR 2) is what admits the 1080p renditions through `capLevelToPlayerSize`; at DPR 1 the same 744px player is held at 720p, which is why phones never see it. The fix, on the `<MuxPlayer>` in `ClaimWatch.jsx`: `preload="auto"` **for everyone but coarse-pointer devices (FOUNDER DECISION, 15 September 2026, PR #12 follow-up, worded as the rule is: coarse pointers — phones, tablets — keep the element's default `metadata`, so no phone pulls tens of MB before play; everyone else preloads, a fine pointer and a browser reporting no pointer alike (CI's headless Firefox reports `pointer: none`) — `PLAYER_PRELOAD`, decided once at module level from `matchMedia('(pointer: coarse)')`)** (the buffer fills during the 10–60 s a desktop reader spends on the page — 70 s of film buffered 8 s after the page opens, measured) and, for EVERY device, `_hlsConfig={HLS_CONFIG}` = `{ maxBufferLength: 60, maxMaxBufferLength: 120, abrBandWidthUpFactor: 0.5 }` (a 60 s cushion; the up-switch to 7 Mbps now needs a 14 Mbps estimate). Resolution is NEVER capped — every rendition stays reachable (`capLevelToPlayerSize` is mux's default and unchanged). `lowLatencyMode` is left at hls.js's default: inert for on-demand playlists (every use in hls.js 1.6 is gated on a live/part playlist). Measured after the fix on the built bundle at DPR 2: 70 s buffered before play, minimum 68 s ahead across three 180 s runs, zero stalls (before: 5 s / 5 s / zero stalls on this fast connection — the emulator's smooth throttling never produced a visible freeze even before; the mechanism above is the evidence, and the founder's own Chrome is the confirmation). **Red-team findings, 15 September, and the founder's rulings:** (a) DATA — with `preload="auto"` a page downloads up to hls.js's 60 MB `maxBufferSize` before play is ever pressed (≈41 MB at 720p, ≈60 MB once at 1080p; ~1.5 MB before). **RESOLVED by the founder the same day: coarse pointers keep `metadata` and never pay it; everyone else preloads (above).** iOS Safari (native HLS) ignores both props regardless — no cost, no fix there. (b) ABR — with the up-factor at 0.5 a connection whose estimate sits between ~7.4 and 14 Mbps stays below the 7 Mbps rendition (1080p @ 5.3 Mbps needs 10.5 Mbps); this is headroom, not a cap. Riders held: `handlePlay`/`handleEnded` (phone fullscreen) and the resume path are byte-identical to `main`; mux's own `startTime` handling already flips `preload` around the resume seek on the native path. `e2e/player-frame.spec.js` pins the attribute on both pointer kinds (a touch context is a coarse pointer on all three engines) and the config that reaches hls.js — both ride library internals a caret upgrade could drop silently — branching on whether the PAGE reports an hls.js instance, never on the engine's name (local desktop Safari plays HLS natively; CI's Linux WebKit constructs hls.js).
...
3. **Within a fan — A FAN NEVER BALLOONS (founder law, 11 September 2026; supersedes the outward-move rules of 10 September).** On the live v4 graph one new invitation flew Arielle's branch far from her, with long lines and hidden names; under the 10 September evening amendment Arielle → Krist read 155px for the same reason (Krist's ten at 48 + 150 units). This must be impossible: a crowded fan solves its crowding IN PLACE, in this order — (1) SPREAD: siblings spread only as far as the clearance rules require (6px between real rendered name boxes, name-vs-dot, name-vs-line, and — new — line-vs-dot: a line keeps 0.6 of the clearance from every dot it is not attached to), never wider than `FAN_MAX_SPAN` = **140°** (was ~120°); every adjacent pair's gap is the SMALLEST at which that sibling clears the parent and the siblings before it (a sweep, `spread`/`smallestGap`/`pairClear` — the incremental widening of 10 September left a few wide gaps and the rest crammed at the cap once the outward move was capped); (2) STAGGER: the fan's LEAVES alternate between the reach distance d and d × `STAGGER_RATIO` = **1.55**, two tight rows around the parent (`row` per node: 0 = near, 1 = far); a sharer always keeps the near row, so the reach ORDER between a parent and its own children — every child beyond its parent — always holds; the alternation runs in one of four orders (starting near or far, straight or mirrored about the fan's middle: on a horizontal limb a near name runs outward along the limb, so the far neighbour must sit on the side its line does not cross) and the order that leaves the fewest of the fan's own rules broken wins; the stagger is decided ONCE by the fan's own names, never toggled by a neighbour's pressure; (3) THE NAMES move (out → in → perpendicular; a perpendicular name now HANGS along the limb — a leaf's outward, a sharer's inward, the other way when neither clears — never centred on the perpendicular, where a 60-unit name straddled both neighbours' lines; a name no side clears asks its own fan for room around it); (4) only then does the whole fan move OUTWARD, by at most `EXTRA_MAX` = **0.5 × d** — d the fan's smallest reach distance (a leaf's 61 → 30.5 units) — a HARD cap (`extra`, reported per node); (5) below that the names shrink down the ladder (§ Name size) and only at its bottom does a name hide. Every other outward trigger is DELETED: the wedged sharer's branch, the stall against the first ring. Fans of different parents that would collide are still separated by the LEAST MOVEMENT (each turns about its own parent, equally, damped). **On Circles (39 tickets, 11 September) at the founder's desktop: Ien → Arielle 95px, Arielle → Krist 143px** (Arielle's eight take the capped 24-unit move; Krist's ten sit 30.5 units out, in two rows of 5/5), Krist → Alexander 110px; Krist's ten span 140°.

**THE FIRST RING GROWS WITH ITS COUNT (founder, 11 September):** its radius is at least count × `RING_ARC` (34) / 2π map units — 34 units of arc per person at the dots — so twenty-plus first-ring names never collide (tomorrow's screening adds cast and crew to the first ring in one day); v4's 118 when that is larger; the existing bump loop still pushes further only if the names cannot clear (on Circles + 15 first-ring people: 25 slots at 227 units).

**STABILITY (founder, 11 September — the target, measured, not yet met in full):** adding one person anywhere should move no existing dot by more than 20% of its distance from the centre and change no OTHER fan's row structure; adding fifteen first-ring people should rotate the first ring evenly and nothing else. Measured on Circles today (`src/lib/constellationGrowth.test.js`, pinned as KNOWN GAPS): no other fan's rows change for any single addition (the fan the person joins, and the fan of a leaf who thereby becomes a sharer and moves to the near row, may); the worst dot move is 12% for one addition and 21–34% for eight of nine — the size ladder re-chooses the whole film's rung (11 → 9) and every fan's gaps change with it, and the relaxation turns a neighbouring fan; +15 first-ring people re-pattern Alexander's stagger (the one fan whose rows change) and drift Krist's children in Arielle's frame. Reported for the founder; the rule stands as the target.
...
**Name size — SHRINK BEFORE HIDE (founder, 11 September 2026).** The size is chosen PER FILM: the largest of **11 / 10.5 / 10 / 9.5 / 9px** (`LABEL_SIZE_LADDER`, `src/lib/constellationLabels.js`) at which every name paints at rest on the founder's desktop — the layout walks the ladder from the top and keeps the first rung whose plan settles with no name lost (`plan.labelPx`); names hide only below 9px, never at 9 or above. When no rung is clean, the rung that loses the fewest names (hidden by the layout, or still colliding for the renderer to hide) holds, the larger size on a tie — the builder's reading, pending the founder's word. The phone follows the same ladder at its own width: the renderer re-walks it at every view (`pickLabelSize`, translation-invariant, so panning never changes the rung) and paints the largest rung at which every name the layout did not itself hide fits; the filmmaker's two center labels ride the same rung. Measured 11 September: Circles today paints at **11px with 3 of 39 names lost** at the desktop (2 hidden, 1 colliding — Krist's ten are at the cap in two rows and at the outward limit); the fixed-size history: 11 (10 September), 10.5 (its evening amendment), 9.5 (v4), 9 (v5). `REACH_K` is **31** (was 18): with the outward move capped the limb's length is the reach rule alone, and the founder's px targets fix it (30 + 31 × 4 = 154 units = 124px on the 33-ticket tree; today's eight-wide Arielle fan adds its capped move: 143px).
...
**Dense films.** When a film outgrows what its reference view can hold under the rules at the bottom of the ladder, the layout reports it (`plan.settled = false`, `plan.hidden`, `plan.colliding`) and the map hides the names that would touch, zoom revealing them — the existing safety net, and the ONLY whole-graph name policy (founder, 10 September: the opening view for large films is decided later, with real numbers). A plan that lost names still settles on a CONSISTENT canvas when it can (the survivors paint at the planned size — the old base-canvas fallback painted them larger and hid many more); a fan of more than 16 children is HOPELESS at every rung and skips the ladder's steps. **The growth table (11 September, `constellationGrowth.test.js`, reference view; every scenario pinned as a KNOWN GAP against the target "settle at ≥ 9.5px, 0 hidden"):** (i) Circles today 39 people · 11px · 2 hidden + 1 colliding · longest segment 143px (2.9 leaf reaches) · (ii) +1 first ring · 10px · 2 + 2 · 148px · (iii) +15 first ring 54 people · 10.5px · 11 + 0 · 158px (the first ring at 227 units) · (iv) +15 first ring, four share 3 each, 66 people · 9.5px · 9 + 5 · 152px · (v) Stacy +3 · 9px · 3 + 0 · 133px · (vi) four quiet first-ring +4 each · 9px · 4 + 2 · 129px · (vii) Krist's other nine +2 each · 9px · 7 + 3 · 170px · (viii) one leaf under each of ten people · 9px · 10 + 1 · 145px (the modal's painted counts for iii and iv are in the render geometry files). A plan that cannot settle keeps its LAST placement round (canvas fitted to its own drawing) rather than re-placing on the base canvas — the base-canvas fallback painted a 1,300-unit drawing's names at a 900-unit canvas's size and hid 21 of 54 names on tomorrow's first ring; the last round paints 38. The "longest segment ≤ 2.5 leaf reaches" target cannot hold on any tree containing Krist: his own limb is the reach rule's √16 = 4 → (30 + 31 × 4) / 61 = 2.52 before Arielle's fan moves at all — the reach rule wins, reported.
...
13. **BUILDER FIX, 15 September 2026 (branch `player-stalls`) — the player's black strip and the first-minute freezes.** (a) `block` on the `<MuxPlayer>` className: the custom element's inline-block baseline gap painted ~9.5px of the wrapper's black under the film (§3a; `e2e/player-frame.spec.js` measures wrapper = player at 1440 and 390). (b) `preload="auto"` for everyone but coarse-pointer devices (founder decision, same day: coarse pointers keep `metadata`; everyone else preloads) + `_hlsConfig` for everyone (60 s cushion, up-switches at twice the headroom): the measured class-A mechanism and the remaining open note (the 7.4–14 Mbps band) are in §3a. Nothing a viewer reads changed; the slug-path baseline holds without re-recording; film and slug modes share the one element.
