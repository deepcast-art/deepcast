...
13. **BUILDER FIX, 15 September 2026 (branch `player-stalls`) — the player's black strip and the first-minute freezes.** (a) `block` on the `<MuxPlayer>` className: the custom element's inline-block baseline gap painted ~9.5px of the wrapper's black under the film (§3a; `e2e/player-frame.spec.js` measures wrapper = player at 1440 and 390). (b) `preload="auto"` + `_hlsConfig` (60 s cushion, up-switches at twice the headroom): the measured class-A mechanism and the two founder decisions it leaves open (pre-play data on Android cellular; the 7.4–14 Mbps band) are in §3a. Nothing a viewer reads changed; the slug-path baseline holds without re-recording; film and slug modes share the one element.
