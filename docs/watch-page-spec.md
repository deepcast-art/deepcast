...
**Dense films.** When a film outgrows what its reference view can hold under (c) at the legible size, the layout reports it (`plan.settled = false`, with `plan.reason` since v5) and the map falls back to hiding the names that would touch, zoom revealing them — accepted by the founder as the safety net (9 September), never the expectation.

### v5 — the founder's evening decisions of 9 September 2026 (branch `constellation-v5`)

**Line meaning (founder law).** A SOLID line is a connection that has arrived — the recipient claimed; a DOTTED line is an invitation still in flight. It pairs with the dot rule (solid = claimed, hollow = not yet): every edge carries `arrived` from the layout, the same claimed-stage fact as the dot at its end. The generation rings stay dotted. So a viewer's gold thread is solid gold end to end, and their own in-flight invitations hang off YOU as dotted gold runs to hollow gold dots. Recede and draw order unchanged.

**Rings are generations (founder reversal of v4's level mechanism).** A person's dot sits EXACTLY on the dotted ring of their generation — never between rings, never stepped out; the "step out one level" resolution is gone. A fan that needs more room than the tight step spreads ALONG its ring, centred on its parent, nudging neighbouring fans around the ring: a one-dimensional circular packing per ring — least movement, two overlapping neighbours moving equally, an untouched fan never leaving its parent's angle, every fan's reach measured from its real name boxes plus the 6px clearance. If a ring's demand exceeds its circumference, THAT ring's radius grows (and every ring outside it moves out by the same amount) until it fits — the dots' fixed step never tightens while the names' reach can still shrink with the radius; the step tightens to the clearance minimum only when the dots themselves cannot fit. A name on the ring that still cannot clear an inner ring's name, dot or line — after turning inward (law (c)) and, once its fan is already at the tight step, after asking its parent's fan for room — grows the ring the same way. Ring spacing is never less than a dot, the label offset, a name box and the clearance, so adjacent rings cannot touch radially where names stack; the growth rule keeps names off the next ring everywhere else. The first ring stays even in ticket order; the filmmaker at the exact centre. (A size-weighted push — the smaller fan yielding more — was tried and dropped the same evening: it broke the packing's convergence.)

**The creator's phone** opens the way a viewer's does: on the film node and the whole first ring with their planned name boxes (`layout.firstRingFrame`, symmetric about the filmmaker), at no smaller a scale than the plan's, centred on the filmmaker; pan, pinch, + / − / 1:1 to the rest.

**Lines never vanish.** Every edge is painted with a screen-pixel stroke (`vector-effect: non-scaling-stroke` — at least one device pixel at any zoom, the dash in screen pixels too) at no less than the recede opacity (`LINE_OPACITY_FLOOR` = `RECEDE_OPACITY`, the edge's own stroke alpha; the receded group still dims it with everything else off the thread — the founder tunes either knob), so the 1:1 phone view reads as connected dots, never a dot-cloud.

**Two calls left open in v4, decided:** an explored lineage on the viewer surface paints at full strength while explored — the recede is applied per element (each off-thread person and segment carries its own opacity), so an explored element lifts to 1 in place and group membership never changes on hover (a first cut moved explored people into their own layer, which re-mounted them and killed the gold fade — red team, 9 September); and a hollow dot's stroke counts as part of the obstacle (`PERSON_DOT_OBSTACLE_R` = the dot radius plus half the hollow stroke — applied to every dot, solid ones too, so the geometry never depends on who has claimed).

**Name size under the ring rule: 9px** (was 9.5 in v4). The founder asked for the largest size at which Circles settles; measured, that is 9px — 9.25 and above diverge. The physics: rings are shared by a whole generation, so the ring Arielle's seven need at the SIDE of the map (≈345 units: their name-bound fan's lines must clear Marcus's first-ring name, which cannot turn inward past the FILMMAKER label) is also the ring Oliver's three sit on at the TOP — the canvas is taller than v4's (v4 let only Arielle's branch step out sideways), the height-limited reference view paints it smaller, and above 9px the font feedback no longer closes.

**What does not settle under the ring rule (reported, never hidden — `plan.reason` names the cause):** (1) Stacy sharing three settles at 8.5px, not 9 (Circles itself is at the edge at 9 — five plan rounds); (2) four quiet first-ring people sharing four each SETTLES at 9px (v4 did not; ring 2 grows to ~713 and the canvas to ~2400×1730 — settled, at the cost of zoom); (3) Krist's other nine sharing two each settles at no floor of 7px or above (v4 failed it at 8.5 and above); it settles at 6px: 23 people on ring 4 around Krist's angle at the side must occupy ~85° of that ring, which spreads them toward the vertical axis, the canvas becomes as tall as ring 4, and the names grow faster than the canvas can hold them. (4) A structural finding the founder must know: ANY third-generation person near 12 or 6 o'clock over a deep sideways branch — the e2e's own Circles-shaped tree (Otis's Juno at the top over Priya's seven and Lena's ten) — settles at no floor of 7px or above (it settles at 6px), because that person rides ring 3 wherever the sideways branch pushes it and the canvas height follows; on Circles today this happens the day one of Oliver's three (Steve, Brian, Katie, at 12 o'clock) shares onward (measured by the red team: Circles + Steve sharing once settles at 7px, + Brian sharing once at 7px). The safety net then hides the few names that would touch at the desktop view (six of thirty on that tree) until zoom. **Settle rate under v5** (the red team's 100 random seeder-shaped trees, 9 September): 20–39 people 15 of 16 settle, 40–59 people 16 of 22 (v4: 2 of 27), 60 and more 7 of 62 (v4: none) — the usual reason above 60 is "the first ring grew to its limit". **Cost:** the layout runs synchronously on the dashboard's main thread; measured on the founder's Mac — 37 people 30–50 ms, 58 people 150–450 ms, 100 people 0.7–2.3 s, 150 people 1.5–4 s. A plan whose canvas grows by more than 8% in three consecutive rounds is abandoned early as diverging (v5, after the red team) — the plan rounds are otherwise the cost. A film past ~100 people will need the layout off the main thread; not built.

**Measured on the `-v5` renders (chromium, real text boxes, the ONE sanitized fixture `screenshots.local/constellation-fan-2026-09-09/circles-fixture.sanitized.json` — 37 rows, no emails; files `v5-*.png` + `.geometry.json`):** the plan settles in 5 rounds on a 1,347×855 canvas (the unit tests' Circles-shaped tree, with slightly different names, settles at 1,376×855); rings 118 / 345 / 467 / 574 with every dot exactly on its ring (worst deviation 0); 17 solid lines (arrived) and 20 dashed (in flight); names at 9px; every edge's own stroke alpha is 0.5 — on a viewer's dashboard the per-element recede halves it again to 0.25 effective off the thread, on the creator's surfaces it paints at 0.5 (the founder's knob, both ways).

| Render | Painted | Tightest name–name | name–dot | name–line |
|---|---|---|---|---|
| creator modal, desktop 1440 | 37/37 | 6.90px (Steve / Brian) | 8.12px (Alexander / Monika's dot) | 9.65px (Marcus / a line) |
| creator modal, phone 390 (opens on the first ring) | 37/37 | 14.62px (Daniel / Patti) | 16.50px | 21.02px |
| Arielle's dashboard, desktop | 37/37 | 6.90px | 8.12px | 9.65px |
| Arielle's phone (opens on her thread) | 37/37 | 9.58px | 13.05px | 13.08px |
| Krist's dashboard, desktop | 37/37 | 6.90px | 8.12px | 9.65px |
| Krist's phone (opens on his thread) | 37/37 | 6.90px | 8.12px | 9.65px |
| Krist's phone at 1:1 (the whole graph) | 7/37 (30 hidden until zoom) | 8.72px | 6.14px | 6.90px |

Every painted category clears 6px on every surface; the 1:1 phone view paints every line (screen-pixel strokes) as connected dots.
