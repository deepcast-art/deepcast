...
5. `margin-top: 2.25rem` (the column's widest gap — the act keeps its isolation) → **the CTA**: `Pass it on` — full rail width, min-height 52px, **solid accent fill, ink text**, caps 0.8125rem/0.28em, border same accent as fill at rest (rest is seamless solid gold). **Hover keyline (FOUNDER-APPROVED 2026-07-25, deliberate — do not "fix"):** on hover the FILL dims to `rgba(177,161,128,0.88)` while the border STAYS solid accent, so a thin brighter ring frames the inside edge — the printed-ticket look. (This replaced the original hover, which dimmed fill AND border together; its alpha compositing happened to draw the same ring, but incidentally. The ring is now explicit and engine-independent.) Focus-visible: 1px accent outline, 3px offset. This is the page's ONLY solid-filled object. Opens the modal.
