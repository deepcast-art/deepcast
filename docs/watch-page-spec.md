...
2. **Reach:** every other person sits BEYOND the person who gave them the film, inside a fan centred on that parent's outward direction (from the centre through a first-ring parent; for a deeper parent, the direction from their own parent through them). Distance from the parent = `REACH_BASE + REACH_K × √(size of the person's own subtree)` — themselves and everyone below — with `REACH_BASE` = 30 and `REACH_K` = 16 map units (a leaf sits 46 units from its sharer; Krist, subtree 16, sits 94 from Arielle before any fan move). Someone who shared with nobody sits close to their sharer; someone whose branch grew sits further out, so a big sharer's branch reads as a long limb: on Circles Ien → Arielle → Krist → Alexander is one limb, and Marcus, Jan and the two Evans stay close to Ien. K was tuned so Arielle → Krist comes out about 20% shorter than the founder's sketch drew it with Arielle roughly midway (measured at the founder's desktop: Ien → Arielle 85px, Arielle → Krist 104px — the second includes Arielle's fan moving outward 50 units for her seven names).
3. **Within a fan,** siblings spread only as far as the clearance rules require — 6px between real rendered name boxes, name-vs-dot (a hollow dot's stroke included), name-vs-line — each adjacent pair's gap on its own; a fan is never wider than `FAN_MAX_SPAN` (~120°); if a fan cannot fit within that, the whole fan moves outward from its parent (`extra`, reported per node) until it does. Fans of different parents that would collide are separated by the LEAST MOVEMENT — each turns about its own parent, the two equally, damped, until the rule holds — never by changing anyone's distance rule. v4's "step out to the next level" is gone. **The four triggers of a fan's outward move (`extra`) — every one "this fan cannot fit where it is", named so the founder can strike any (red-team finding, 10 September):** (a) its own names need more than ~120°; (b) a sibling's line would be swallowed by its own two name boxes; (c) a sharer's branch is WEDGED between that sharer's own siblings and the outer fan has already spread to its cap — the outer fan moves, so the same angular gaps span more room; (d) a fan is wedged against what cannot move (the film node's labels, a first-ring name or line) and turning has stopped helping. Nothing else changes a distance. On Circles the override is real: Krist's ten sit 46 + 120 units from him (trigger a), Arielle's seven 46 + 50, Alexander's five 46 + 80 — the reach ORDER inside every fan survives (Krist further from Arielle than her leaves; Alexander further from Krist than his). Every clearance rule of v4/v5 stays, measured on real rendered boxes; the label-side rules stay (a name never sits on a line it isn't attached to — it turns to the inward side of its dot when that is clear, a sharer preferring inward — nor on its own dot's line).
...
**Name size.** Names paint at the readability floor on every surface, against the map's TRUE scale: **11px** — the largest size at which Circles settles at rest under the hard clearance rule (measured on the sanitized fixture: 9.5, 10, 10.5, 11 and 11.25 settle; 11.5 and above fall back; 11 was chosen as the round value one step inside the edge — the founder can move it to 11.25).
