...
test.describe('three-page claim arc', () => {
  ...
  test('the landing letter: still, thread, hook, inline email — and nothing that belongs elsewhere', async ({ page }) => {
    ...
    // Inline email — visible immediately, no click-to-reveal. The claim
    // form is EMAIL-ONLY since 2026-08-06: the "Your full name" field
  ...
  test('full arc: claim → prologue → the redesigned watch page → revisit', async ({ page }) => {
    ...
    expect(claimBody.fullName).toBeNull()
    ...
    await expect(page.getByText('People have been gifted this film of 1,000 goal')).toBeVisible()
