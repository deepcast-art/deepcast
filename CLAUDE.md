...
- `POST /api/invites/create-link` — two sharer-identity paths (both set `parent_invite_id`, unified lineage model): a **verified session** (Bearer → `getUser()`, reusing the exact parent-resolution fallbacks and quota rules of the legacy send route), or a **claimed invite reference** for accountless sharers (`server/claimIdentity.js` — their claim IS their identity; their ticket spend is CAS-decremented, see Tickets). Slug generation in `src/lib/../server/inviteSlug.js`: Unicode-normalize → strip to a-z → max 20 chars → fallback `invite`; reserved-route blocklist; 4-char unambiguous suffix (no 0/o/1/l/i), 3 retries then widen to 5.
...
- `POST /api/invites/claim` — the email IS the claim. Atomic race-guarded UPDATE (`eq status 'created'` + `is claimed_email null`); stamps `claimed_email`, `claimed_at`, `status='claimed'`, the frozen `claim_ordinal`, and `tickets_remaining = 5`. The sharer's own session never claims; the film never travels back to its maker (`server/shareRules.js`). Routes DIRECTLY to the watch page — there is no reveal beat.

### Tickets (dual backing, one economy — decided 2026-07-16)

Spent at link GENERATION, no refunds (a failed generation is refunded — that's not a spend). Account holders: `users.invite_allocation` machinery unchanged, surfaced as "tickets". Accountless claimants: `invites.tickets_remaining` on their OWN claimed invite, initialized to 5 at claim, CAS-decremented by create-link (`src/lib/ticketRules.js` — NULL means claimed-pre-migration and heals to the full grant on first spend). Zero state: "You've given all your tickets for this film." — no upsell.
...
A claimant's identity is their claimed invite row. The claim response is stashed via `src/lib/claimStash.js` (safeStorage; degrades to in-memory for the visit under Safari restrictions). `/dashboard` admits EITHER an auth session (existing ProtectedRoute + ViewerShareGate path, untouched) OR a claim stash (`DashboardGate` in `App.jsx`); claimant mode synthesizes a viewer-shaped pseudo-profile and hides name-edit, sign-out, About, and the email share modal. **RULE: no identity state may ever render a blank page** — unresolved states get a spinner; unidentified visitors get the graceful "This page belongs to invited viewers." screen (`profileLoaded` is FALSE forever for sessionless visitors — never bare-guard on it). Revisit rule: the owner re-opening their claimed slug (stash match) routes to their watch page; everyone else gets the dead-link page; stash-less new browser → dead-link is the ACCEPTED MVP limitation, no recovery flow.
...
## CURRENT STATE (2026-07-16 — read this before continuing any open item)

**Built, verified, and live on branch `invite-rework`** (owner pushes manually; every commit listed in `git log`): the full three-page claim arc above, end to end — landing letter (film still, thread, inline claim) → watch (share panel, tickets, second-generation link) → claimant dashboard (graph, frozen ordinal). Two applied migrations beyond the base schema: `20260716_films_transmission_hook.sql` and `20260716_tickets_ordinal_poster.sql` (plus `20260706_invites_claim_link_fields.sql`). Verification bar at last green: 247 unit tests, e2e on chromium+webkit+firefox (three-page arc, restricted storage incl. the claim flow, never-blank dashboard), console clean on the live happy path. D4 graph cleanup executed 2026-07-16 with owner approval (test nodes removed; 49 ghosts + Jon Bregel + filmmaker rows intact).

**Open items:**
- **A5** — retire the legacy email-invite CREATION surfaces (`InviteForm.jsx`, pass-it-on letter, dashboard email modal). The `/i/:token` acceptance path and email templates stay forever (links never expire).
...
**Accepted MVP limitations (by decision — do not "fix" without Ien):** stash-less revisit of a claimed link shows the dead-link page (no recovery flow); two same-first-name claimants on one film can merge into one graph node; legacy account-viewer screening cards show no progress bar (seconds saved, no fraction); claimed-but-unwatched does not count toward reach.
...
**Standing doctrines in force for all of this work** (details elsewhere in this file): commit-only, owner pushes; migrations are prepared as files and applied by the owner via the SQL editor, never by Claude; secrets referenced by variable name only, never printed; production deletions are dry-run first, shown row-by-row, owner-approved, backed up, explicit id allowlists with protected-email refusal guards (protected: filmmaker@gmail.com, jbregel@gmail.com — Jon Bregel, A Sacred Pause's director, the realest node on the graph — contact@tracebelll.com, and the other real users listed in `server/reset-test-data.js`).
...
- **Shares used** (dashboard) = the viewer's sent-invite rows for the selected film (list length — counts the same rows the sent-list shows). **Films / N invites / Show more (N remaining)** are plain lengths of the displayed lists themselves. Viewer-facing labels say **"Tickets given" / "Tickets left"** (2026-07-16); the computations are unchanged.
- **Tickets** (`src/lib/ticketRules.js`): the accountless claimant's quota — `INITIAL_CLAIMANT_TICKETS` (5) and `ticketSpendDecision` (the server's CAS spend uses it; NULL heals to the full grant). Account holders still flow through `invitationsRemaining`.
