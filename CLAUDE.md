...
- **"The Faith Dialogues"** (`6a9c0c79-24f6-427e-ba34-c113acf92d9f`, mux playback `4HnHRG3NAf9YYR7V1fNs0143gGJnLUZ9F1umQuXsOaaQ`, mux asset `maKIC023p9tTzstYdjC7mbMPd6nAOA6iDacDBSm00uwss`, 33m03s / 1983s) — **PRIVATE DEMO film** (row inserted by the owner 2026-07-22), `show_ghosts` TRUE; its ~50 ghosts are seeded by `server/seed-faith-ghosts.js` (dry-run by default, idempotent — aborts if the film already has ghosts, owner-run only, `…fd##@demo-deepcast.invalid` recipients, never ticket-numbered). `gif_start`/`gif_end` are NULL by design (the GIF mechanism is retired — see the email section).
...
2. **Watch `/watch/{slug}`** (`src/pages/ClaimWatch.jsx`): **REDESIGNED 2026-07-23/24 (watch-redesign-2) — THE BINDING SPEC + full amendment record is `docs/watch-page-spec.md`; read it before touching this page** (pixel ground truth `design-refs/watch-page_24.html`, local-only). Desktop ≥900px: header (wordmark + "Your dashboard →") / masthead (title, explicit italic + "14 minutes. Headphones recommended." via `filmConditionsLine`) / the hero grid — the Mux player left (poster from `src/content/filmStory.js` per-film — Mux thumbnail fallback, one-line override; flat on ink, never autoplay), the RAIL right: the tier bar toward the next ladder goal (`src/lib/viewerTiers.js` — fixed 100→1,000,000 ladder, no percentages ever), the TRUE film-wide viewer count (`filmClaimsCount` from the link route, shared rule `src/lib/filmClaims.js`), "Milestones passed" hallmarks (the WHOLE block absent until the first tier — no placeholder, no celebration; separators are spacing only), the gold "Pass it on" CTA (the page's ONLY solid-filled object), and the pairs-of-hands rule line (`src/lib/handsChain.js`, "1 pair" singular, "— or its last." in accent, NBSP-bound). Below: the three-statement creed band and the per-film filmmaker story (`src/content/filmStory.js` — TEMP placeholder copy clearly marked, real names, TEMP locations; a film with no entry renders NO story section). **The pass-it-on flow lives in a MODAL opened by the CTA** (founder override of the 2026-07-19 always-docked decision): native <dialog>, the lineage emblem (fixed geometry, REAL first names, near-branch forks lit ONLY by the server's `lineageForks` booleans — `src/lib/lineageForks.js`, never invented people; INTERIM translate-centering for chains <3 pending the sparse design-ref), ticket stubs, "{n} tickets left.", the charge, the first-name form, and the REPLACEMENT-MODEL reveal (the link renders where the field was — zero scrolling; "Create another invitation" cycles the window; "See where your ticket went →" exits to the dashboard). **The personalized constraint line is CUT from this page** — `constraintLine.js` stays lib-only with no rendering surface; its designated future home is the claim/invitation flow. Unchanged by the redesign: the arrival fade (`.dc-watch-arrival`, in-memory marker), resume via `resumePositionToSave` (slug-scoped keys, near-end ERASED, `?again=1` starts clean), the ≥70% watched mark, and phone fullscreen/rotate-on-play.
...
- **Mechanism is materialize-on-write, NEVER resolve-at-render.** RLS makes render-time resolution impossible (users rows are readable own-row/team-only — verified in `pg_policies`), and a second resolution path would let surfaces disagree. `invites.recipient_name`/`sender_name` are the ONE render source every surface reads (unchanged readers: `displayName.js`, `ticketRows.js`, `constellationLayout.js`, `graphLayout.js`, `networkPeople.js`, the server lineage builder); `users.name` is the authority that writes them, at exactly three moments:
  ...
  2. **Claim** re-stamps `recipient_name` from an ATTACHED account's current name (`server/claimNameRule.js`, unit-tested; created accounts are born matching; blank or @-containing account names never stamp — an email fragment is never written as a name).
...
- Known leftover: The Faith Dialogues ticket №2 was typed "Kanye" and claimed by the `ien.chi96+test18` test account before this rule shipped — left as-is deliberately (owner decision 2026-07-23); it goes away in a future by-hand test-data cleanup. Note that if that account's name is ever edited, the fixed propagation will rename the demo node.
...
- Reveal (STAMPED 2026-07-23, superseding the 2026-07-21 wording): the watch modal's line 1 is "Here's {name}'s ticket link. Send it to them with why they came to mind." (the dashboard share modal keeps its own line 1). The tickets line (`revealTicketsLine.js`, ONE rule for BOTH share surfaces — the dashboard modal picking up the new copy was owner-intended): "{N} tickets left. Who else needs it?" / "1 ticket left. Who else needs it?" / N=0: "That was your last ticket for this film." / unlimited: "Who else needs it?" (numerals always). New watch-page founder copy (all verbatim, ledger in `docs/watch-page-spec.md` §7): "Pass it on" · "Pass it on. Make an impact." · "Viewers reached of {goal} goal" · "Milestones passed" · "This film passed through {n} pair(s) of hands to reach you. You are its newest link — or its last." · the three creed statements · "Who needs to see this? Not anyone — the one it will matter to." · "{n} tickets left." · "Create another invitation" · "From the filmmaker · {location}". **The "one uniform type style" rule is AMENDED (not deleted): exactly three approved in-copy emphasis exceptions, all on the watch page — "or its last." (rule line), "you" (creed 2), "needs" (the charge).**
...
- **Watch-page redesign stats (2026-07-23/24)** — `src/lib/viewerTiers.js` (the fixed tier ladder: next goal, crossed milestones, bar fill — never rendered as a percentage), `src/lib/filmClaims.js` (the film-wide viewer count — served by the link route, shown on the watch rail), `src/lib/handsChain.js` (chain depth, "1 pair" singular grammar, emblem labels + 8-char cap, fork-flag alignment), `src/lib/lineageForks.js` (which hands verifiably shared onward — powers the emblem's forks, server-side). New surfaces read these — never inline math.
